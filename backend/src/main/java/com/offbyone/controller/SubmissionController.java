...
public class SubmissionController {
    ...
    public SubmissionController(SubmissionRepository s, ProblemRepository p, RoomRepository r, JudgeService j) {
        this.submissionRepo = s; this.problemRepo = p; this.roomRepo = r; this.judgeService = j;
    }
    ...
    public ResponseEntity<?> submit(@RequestBody Map<String, String> body, @AuthenticationPrincipal User user) {
        ...
        sub.setUser(user); sub.setProblem(problem);
        sub.setLanguage(body.get("language")); sub.setCode(body.get("code"));
        if (body.containsKey("roomId"))
            roomRepo.findById(UUID.fromString(body.get("roomId"))).ifPresent(sub::setRoom);
    ...
    public ResponseEntity<Submission> get(@PathVariable UUID id) {
        return submissionRepo.findById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
