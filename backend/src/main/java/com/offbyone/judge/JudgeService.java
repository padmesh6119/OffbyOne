...
public class JudgeService {
    ...
    public JudgeService(SubmissionRepository submissionRepo, TestCaseRepository testCaseRepo, SimpMessagingTemplate ws) {
        this.submissionRepo = submissionRepo; this.testCaseRepo = testCaseRepo; this.ws = ws;
    }
    ...
    public void judge(Submission submission) {
        ...
        ws.convertAndSend("/topic/submission/" + submission.getId(), Map.of("verdict", verdict, "runtimeMs", totalRuntime));
    ...
    private RunResult run(String code, String language, String input, int timeLimitMs, int memoryMb) {
        try {
            ...
            if (compileCmd != null) {
                ...
                compile.waitFor();
                if (compile.exitValue() != 0) { cleanup(tmpDir); return new RunResult("ce", "", 0); }
            }
            ...
            Process run = new ProcessBuilder("sh", "-c", runCmd + " < input.txt").directory(tmpDir.toFile()).start();
            ...
            if (!finished) { run.destroyForcibly(); cleanup(tmpDir); return new RunResult("tle", "", runtimeMs); }
